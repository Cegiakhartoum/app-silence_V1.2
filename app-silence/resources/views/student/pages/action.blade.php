    ...
    <div id="menuToggle">
        ...
        <div id="menu" class="d-flex flex-column align-items-left">
            ...
            <div class="action-menu-item {{ isActiveClass(1) }}">
                <a href="?c=1&p={{ $action->projet_action_id }}" data-href="?c=1&p={{ $action->projet_action_id }}">La
                    thématique</a>
            </div>
            <div class="action-menu-item {{ isActiveClass(2) }}">
                <a href="?c=2&p={{ $action->projet_action_id }}" data-href="?c=2&p={{ $action->projet_action_id }}">Le
                    pitch</a>
            </div>
            <div class="action-menu-item {{ isActiveClass(3) }}">
                <a href="?c=3&p={{ $action->projet_action_id }}" data-href="?c=3&p={{ $action->projet_action_id }}">Le
                    schéma narratif</a>
            </div>
            <div class="action-menu-item {{ isActiveClass(4) }}">
                <a href="?c=4&p={{ $action->projet_action_id }}" data-href="?c=4&p={{ $action->projet_action_id }}">Le
                    synopsis</a>
            </div>
            <div class="action-menu-item {{ isActiveClass(5) }}">
                <a href="?c=5&p={{ $action->projet_action_id }}" data-href="?c=5&p={{ $action->projet_action_id }}">Le
                    traitement</a>
            </div>
            <div class="action-menu-item {{ isActiveClass(6) }}">
                <a href="?c=6&p={{ $action->projet_action_id }}" data-href="?c=6&p={{ $action->projet_action_id }}">Mise
                    en page de ton scenario</a>
            </div>
            <div class="action-menu-item {{ isActiveClass(7) }}">
                <a href="?c=7&p={{ $action->projet_action_id }}"
                    data-href="?c=7&p={{ $action->projet_action_id }}">Scénario</a>
            </div>

            <div class="action-menu-item {{ isActiveClass(16) }}">
                <a href="?c=16&p={{ $action->projet_action_id }}"
                    data-href="?c=16&p={{ $action->projet_action_id }}">Découpage technique</a>
            </div>

            <div class="action-menu-item {{ isActiveClass(18) }}">
                <a href="?c=18&p={{ $action->projet_action_id }}"
                    data-href="?c=18&p={{ $action->projet_action_id }}">Lieux de tournage</a>
            </div>

            <div class="action-menu-item {{ isActiveClass(29) }}">
                <a href="?c=29&p={{ $action->projet_action_id }}"
                    data-href="?c=29&p={{ $action->projet_action_id }}">Liste des acteurs/actrices</a>
            </div>
          
            <div class="action-menu-item {{ isActiveClass(38) }}">
                <a href="?c=38&p={{ $action->projet_action_id }}"
                    data-href="?c=38&p={{ $action->projet_action_id }}">Liste technique et artistique</a>
           </div>


            <div class="action-menu-item {{ isActiveClass(27) }}">
                <a href="?c=27&p={{ $action->projet_action_id }}"
                    data-href="?c=27&p={{ $action->projet_action_id }}">Dépouillement personnage</a>
            </div>

            <div class="action-menu-item {{ isActiveClass(17) }}">
                <a href="?c=17&p={{ $action->projet_action_id }}"
                    data-href="?c=17&p={{ $action->projet_action_id }}#open-modal">Planning de tournage</a>
            </div>

            <div class="action-menu-item {{ isActiveClass(21) }}">
                <a href="?c=21&p={{ $action->projet_action_id }}"
                    data-href="?c=21&p={{ $action->projet_action_id }}">Feuille de script</a>
            </div>
                ...
                <table>
                    ...
                    <tr>
                        <td>
                            ...
                            <div>C'est le problème auquel le personnage principal sera confronté.</div>
                        </td>
                        <td>
...
<br>
        ...
        @elseif($chapterKey == 38)
        {{-- SCHEMA NARRATIF --}}
        @include('student.components.action.liste_technique')
        @elseif($chapterKey == 6)
...
@section('help')
    <div style="text-align: LEFT; width: 200px; ">
        ...
        @if ($chapterKey == 7)
            <br>
            <button id="saveIcon" type="submit" class="btn btn-orange rounded-pill" style="width:100%;">Enregistrer le
                projet</button>
        @endif
        @if ($chapterKey != 21 && $chapterKey != 7)
            <br>
            <button id="saveModal-button" type="submit" class="btn btn-orange rounded-pill"
                style="width:100%;">Enregistrer le projet</button>
        @endif
        <br>
        ...
        @if ($chapterKey == 7)
            <a tabindex="0" data-bs-toggle="dropdown">
                <button type="submit" class="btn btn-orange rounded-pill"
                    style="width:100%; background:#4A4D77; border-color:#617A9A; margin-right: 5px;margin-top:15px;">Ajouter
                    une séquence</button>
            </a>
        ...
        @if ($chapterKey == 17)
            <a tabindex="0" data-bs-toggle="dropdown">
                <button type="submit" class="btn btn-orange rounded-pill"
                    style="width:100%; background:#4A4D77; border-color:#617A9A; margin-right: 5px;margin-top:15px;">Gestion
                    du planning</button>
            </a>
            <ul class="dropdown-menu">
                <li>
                ...
                <li>
                    <a id="add-sequence-menu" class="dropdown-item" href="#open-modal-déjeuner">Pause déjeuner</a>
                </li>
        ...
        <a tabindex="0" data-bs-toggle="dropdown" style="color:#617A9A; text-align:left; font-size:15px;">
            <img src="https://silence-2021.s3.eu-west-3.amazonaws.com/ressources/icon/telecharger.png" alt="Télécharger"
        ...
        <ul class="dropdown-menu">
            ...
            @if ($chapterKey != 0)
                <li><a class="dropdown-item"
                        href="/report/{{ getActionTitle() }}?id={{ $action->id }}">{{ getActionTitle() }}</a></li>
            @endif
            <li>
                <hr class="dropdown-divider">
            </li>
        ...
        @if ($chapterKey == 7 or $chapterKey == 16)
            <br>
            <a style="color:#617A9A; text-align:left; font-size:15px;">
                <img src="https://silence-2021.s3.eu-west-3.amazonaws.com/ressources/icon/oeil.png" alt="Vue classique"
                    id="{{ $chapterKey == 7 ? 'scenario-vue-classique-btn' : 'decoupage-vue-classique-btn' }}"
                    style=" height: 18px;padding-right:25px;" />
                Vue classique
            </a>
        @endif
