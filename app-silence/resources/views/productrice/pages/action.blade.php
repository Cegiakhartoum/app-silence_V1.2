@extends('productrice.layouts.page-action')
...
@endif
    ...
    @else
        @if ($chapterKey == 3)
            ...
            <form id="action-form" class="flex-grow-1" style="overflow-y: scroll;" method="POST" action="/save-action">
                ...
                <input type="hidden" name="redirect_url"
                    value="/distributeur/action?p={{ $action->projet_action_id }}&c={{ $nextChapterKey }}" />
                <table>
                    <tr>
                        <td>
        ...
        @elseif($chapterKey == 6)
            ...
            @include('distributeur.components.action.mise-en-page-scenario')
        @elseif($chapterKey == 7)
            {{-- SCENARIO --}}
            @include('distributeur.components.action.scenario')
        @elseif($chapterKey == 16)
            {{-- DECOUPAGE TECHNIQUE --}}
            @include('distributeur.components.action.decoupage-technique')
        @elseif($chapterKey == 18)
            {{-- LIEU DE TOURNAGE --}}
            @include('distributeur.components.action.lieu_tournage')
      
        @elseif($chapterKey == 29)
            {{-- Liste ACTEURS/ATRICES --}}   
            @include('distributeur.components.action.liste_acteurs')

        @elseif($chapterKey == 27)
            {{-- DEPOUILLEMENT --}}
            @include('distributeur.components.action.depouillement')

        @elseif($chapterKey == 17)
            {{-- PLANNING DE TOURNAGE --}}
            @include('distributeur.components.action.planning')

        @elseif($chapterKey == 21)
            {{-- FEUILLE DE SCRIPT --}}
            @include('distributeur.components.action.feuille_script')
        @else
        @endif
    @endif
...
@section('help')
    ...
    Vue classique
     ...
     @endif
     <br>
     <a href="/distributeur/cours?c={{ $chapterKey }}"style="color:#617A9A; text-align:left; font-size:15px;">  
