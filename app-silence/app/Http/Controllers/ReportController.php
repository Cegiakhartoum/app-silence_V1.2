...
class ReportController extends Controller
{
 ...
 set_time_limit(3000);
    ...
    return response()->download($mergedPDF, 'merged_document.pdf', $headers);
    }
    ...
    public function thématiquePdf(Request $request)
    {
        ...
        try {
            ...
            $pdf = PDF::loadView('pdf.Thématique', $data);
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="thématique.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'thématique.pdf', $headers)->deleteFileAfterSend(true);
...
// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'pitch.pdf', $headers)->deleteFileAfterSend(true);
    ...
    public function shémaPdf(Request $request)
    {
        ...
        try {
            ...
            $data['action'] = $action;
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Shéma', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="shéma.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'shéma.pdf', $headers)->deleteFileAfterSend(true);
    ...
    public function synopsisPdf(Request $request)
    {
            ...
            try {
                ...
                // Generate the PDF
       // Generate the PDF
       $pdf = PDF::loadView('pdf.Synopsis', $data);
       $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="synopsis.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'synopsis.pdf', $headers)->deleteFileAfterSend(true);
    ...
    public function traitementPdf(Request $request)
    {
        ...
        try {
            ...
            $data['action'] = $action;
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Traitement', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="traitement.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'traitement.pdf', $headers)->deleteFileAfterSend(true);
    ...
    public function scénarioPdf(Request $request)
    {
        ...
        try {
            ...
            $data['action'] = $action;
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Scénario', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="scénario.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'scénario.pdf', $headers)->deleteFileAfterSend(true);
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="découpage.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'découpage.pdf', $headers)->deleteFileAfterSend(true);
    ...
    public function lieux_tPdf(Request $request)
    {
        ...
        try {
           ...
           $action = Action::where("id", $action_id)->first();
            ...
            $data['group'] = $group;
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Lieux_t', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="lieu_tournage.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'lieu_tournage.pdf', $headers)->deleteFileAfterSend(true);
 ...
 set_time_limit(3000);
        ...
        try {
            ...
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Liste_acteur', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="liste_acteur.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'liste_acteur.pdf', $headers)->deleteFileAfterSend(true);
 ...
 set_time_limit(3000);
        ...
        try {
          $group=StudentGroup::where("projet_action_id", $action_id)->first();
            ...
            $pdf = app('dompdf.wrapper');
            $pdf->getDomPDF()->set_option("enable_php", true);
            $pdf->setOptions($options)->loadView('pdf.introduction', $data);
  ...
  set_time_limit(3000);
        ...
        try {
          $group=StudentGroup::where("projet_action_id", $action_id)->first();
            ...
            $pdf = app('dompdf.wrapper');
            $pdf->getDomPDF()->set_option("enable_php", true);
            $pdf->setOptions($options)->loadView('pdf.descriptionducontenu', $data);
 ...
 set_time_limit(3000);
        ...
        try {
          ...
          $data['group'] = $group;
            ...
            $pdf = app('dompdf.wrapper');
            $pdf->getDomPDF()->set_option("enable_php", true);
            $pdf->setOptions($options)->loadView('pdf.tonetstyle', $data);
 ...
 set_time_limit(3000);
    ...
    $action_id = $request->query('id',  false);
        ...
        try {
            ...
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Dépouillement', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="dépouillement.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'dépouillement.pdf', $headers)->deleteFileAfterSend(true);
...
// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'planning.pdf', $headers)->deleteFileAfterSend(true);
    ...
    public function feuille_scriptPdf(Request $request)
    {
        ...
        try {
            ...
            // Generate the PDF
            $pdf = PDF::loadView('pdf.Feuille_script', $data);
            $pdf->setOptions($options);
            $pdf->setPaper('A4', 'landscape');
...
$headers = [
    'Content-Type' => 'application/pdf',
    'Content-Disposition' => 'attachment; filename="feuille_script.pdf"',
];

// Return the downloadable file
return response()->download(storage_path('app/' . $localPath), 'feuille_script.pdf', $headers)->deleteFileAfterSend(true);
